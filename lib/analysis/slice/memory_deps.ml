...
let rec find_parameter_value_set 
  ...
  | Some (cfg_pointers, spec, _) ->
    ...
    | "l" :: idx :: rest ->
      ...
      Value_set_abstraction.(+) (find_parameter_value_set ~is_call_indirect pointer_analysis label rest) vs) 
    | "g" :: idx :: "+" :: rest
    | "g" :: idx :: rest ->
    ...
    | "n" :: "e" :: "g" :: lg :: idx :: rest ->
      Value_set_abstraction.(
        (find_parameter_value_set ~is_call_indirect pointer_analysis label rest)
        - (find_parameter_value_set ~is_call_indirect pointer_analysis label [lg; idx]))
    | lst ->
      ...
      List.fold 
        ...
        ~f:(fun acc address ->
          ...
          let store_vs =
            ...
            | _ -> ValueSet address
          in
          Value_set_abstraction.may_overlap ~store_size:1l ~load_size ~store_vs ~load_vs:load_address_value_set) in
...
let call_depends_on_call
  ...
  | Some (_, _, summaries) ->
    ...
    let addresses_read_by_fct1 : Value_set_abstraction.t = 
    ...
    let addresses_affected_by_fct2 = 
      ...
      |> List.map ~f:(fun address ->
        ...
        | RIC {stride; lower_bound; upper_bound; offset=(relative_offset, o)} ->
          let tmp_vs =
            find_parameter_value_set ~is_call_indirect:dependency_is_indirect pointer_analysis depends_on_this_call (tokenize relative_offset)
            |> (Value_set_abstraction.(+)
                (Value_set_abstraction.ValueSet
                  (Reduced_interval_congruence.RIC.ric (stride, lower_bound, upper_bound, ("", o)))))
          in
      ...
      addresses_affected_by_fct2
        ...
        |> List.fold ~init:false
          ~f:(fun acc addr ->
            ...
            acc || Value_set_abstraction.may_overlap ~store_size:1l ~load_size:8l ~store_vs:(ValueSet addr) ~load_vs:addresses_read_by_fct1)
    in
