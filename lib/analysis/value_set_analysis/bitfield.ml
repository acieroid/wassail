...
let%test_module "Bitfield tests" = (module struct
  ...
  let%test "and_ neutral/absorbing" =
    ...
    (* Top neutral, Bottom absorbing *) Bitfield.equal a x && Bitfield.equal b Bottom
  ...
  let%test "or_ absorbing/neutral" =
    ...
    (* Top absorbing, Bottom neutral *) Bitfield.equal a Top && Bitfield.equal b x
  ...
  let%test "join Top/Bottom" =
    ...
    Bitfield.equal a Top && Bitfield.equal b x && Bitfield.equal c x
  ...
  let%test "and_/or_ idempotent on singletons" =
    ...
    Bitfield.equal r_and x && Bitfield.equal r_or x
