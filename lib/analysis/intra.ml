...
module Result (Transfer : Transfer.TRANSFER_BASE) = struct
  ...
  type intra_results = {
    ...
    blocks: (Transfer.State.t * t) Cfg.BlockIdx.Map.t;
  }
...
module Make
  ...
  let analyze_
    ...
    : Result.intra_results =
    let bottom = Result.Uninitialized in
    (* Data of the analysis *)
    let analysis_data : Result.intra_results ref = ref Result.{
      instrs = Instr.Label.Map.empty;
      blocks = Cfg.BlockIdx.Map.empty;
    } in
    let after_block (block_idx : Cfg.BlockIdx.t) : Result.t = match Map.find !analysis_data.blocks block_idx with
      | Some (_, r) -> r
      | None -> bottom in
    ...
    let transfer (b : 'a Basic_block.t) (state : Transfer.State.t) : Result.t =
      ...
      | Data instrs ->
        Simple (List.fold_left instrs ~init:state ~f:(fun prestate instr ->
            ...
            analysis_data := { !analysis_data with instrs = Map.set !analysis_data.instrs ~key:instr.label ~data:(prestate, Simple poststate) };
            poststate))
      | Call instr ->
        ...
        poststate
      | Entry | Return _ -> failwith "Should not have an entry/return in an intra analysis"
      | Control instr ->
    ...
    let rec narrow (blocks : Cfg.BlockIdx.t list) : unit = match blocks with
      | [] -> ()
      | block_idx :: blocks ->
        let result = analyze_block block_idx in
  ...
  let analyze
    ...
    let analysis_result = analyze_ module_ cfg extra in
    let analyzed_cfg = Cfg.map_annotations cfg
        ~instrs:(fun instr -> to_state (match Map.find analysis_result.instrs (Instr.label instr) with
...
module MakeSumm
  ...
  let analyze
    ...
    let analysis_result = analyze_ module_ cfg extra in
    let analyzed_cfg = Cfg.map_annotations cfg
        ~instrs:(fun instr -> to_state (match Instr.Label.Map.find analysis_result.instrs (Instr.label instr) with
            | Some (before, after) -> (Simple before, after)
            | None -> (Uninitialized, Uninitialized)))
        ~blocks:(fun bidx -> to_state (match Cfg.BlockIdx.Map.find analysis_result.blocks bidx with
            | Some (before, after) -> (Simple before, after)
            | None -> (Uninitialized, Uninitialized))) in
    analyzed_cfg
end
...
module MakeClassicalInter (Transfer : Transfer.CLASSICAL_INTER_TRANSFER) = struct
  ...
  type results = {
    instrs: (Transfer.State.t * Result.t) ResultKey.Map.t;
    blocks: (Transfer.State.t * Result.t) Icfg.BlockIdx.Map.t;
  }
  ...
  let analyze_
    ...
    let bottom = Result.Uninitialized in
    (* Data of the analysis *)
    let analysis_data : results ref = ref {
      instrs = ResultKey.Map.empty;
      blocks = Icfg.BlockIdx.Map.empty;
    } in
    let after_block (block_idx : Icfg.BlockIdx.t) : Result.t = match Map.find !analysis_data.blocks block_idx with
      | Some (_, r) -> r
      | None -> bottom in
    (* Applies the transfer function to an entire block *)
    let transfer (b : 'a Basic_block.t) (state : Transfer.State.t) : Result.t =
      ...
      let result : Result.t = match b.content with
        ...
        | Data instrs ->
          Result.Simple (List.fold_left instrs ~init:state ~f:(fun prestate instr ->
              let poststate = Transfer.data module_ cfg instr prestate in
              analysis_data := { !analysis_data with instrs = Map.set !analysis_data.instrs ~key:{ label = instr.label; kind = None } ~data:(prestate, Simple poststate) };
              poststate))
        | Call instr ->
          let poststate = Transfer.call_inter module_ cfg instr state in
          analysis_data := { !analysis_data with instrs = Map.set !analysis_data.instrs ~key:{ label = instr.label; kind = None } ~data:(state, Simple poststate) };
          Simple poststate
        | Entry ->
          if is_imported b.fidx then
            ...
            Simple poststate
        | Return instr ->
          let state_before_entry = match Map.find !analysis_data.instrs { label = instr.label; kind = None } with
            | None ->
          ...
          analysis_data := { !analysis_data with instrs = Map.set !analysis_data.instrs ~key:{ label = instr.label; kind = Return } ~data:(state, Simple poststate) };
          Simple poststate
        | Control instr ->
          let poststate = match Transfer.control module_ cfg instr state with
            | `Simple s -> Result.Simple s
