...
type t = {
  ...
  basic_blocks: BasicBlockIntMap.t;
...
let to_dot (cfg : t) : string =
  Printf.sprintf "digraph \"CFG of function %d\" {\n%s\n%s}\n"
    cfg.idx
    (String.concat ~sep:"\n" (List.map (BasicBlockIntMap.IntMap.to_alist cfg.basic_blocks) ~f:(fun (_, b) -> Basic_block.to_dot b)))
...
let find_block_exn (cfg : t) (idx : int) : Basic_block.t =
  BasicBlockIntMap.IntMap.find_exn cfg.basic_blocks idx

let successors (cfg : t) (idx : int) : int list =
  BasicBlockIntMap.IntMap.find_multi cfg.edges idx

let predecessors (cfg : t) (idx : int) : int list =
  BasicBlockIntMap.IntMap.find_multi cfg.back_edges idx

let callees (cfg : t) : IntSet.t =
  (* Loop through all the blocks of the cfg, collecting the targets of call instructions *)
  BasicBlockIntMap.IntMap.fold cfg.basic_blocks
    ~init:IntSet.empty
